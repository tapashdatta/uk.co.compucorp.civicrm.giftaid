...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/Civigiftaid/DAO/" ] && mkdir -p "$EXTROOT/CRM/Civigiftaid/DAO/"
  cp -f "$CIVIROOT/CRM/Civigiftaid/DAO/"/* "$EXTROOT/CRM/Civigiftaid/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/Civigiftaid" "$EXTROOT/CRM/Civigiftaid/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
